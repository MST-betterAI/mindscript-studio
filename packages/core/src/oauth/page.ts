...
// etc.).
//
// The visual language mirrors the MindScript Studio app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `MindScript Studio is now connected to ${escapeHtml(provider)}.` : "MindScript Studio is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `MindScript Studio couldn't finish connecting to ${escapeHtml(provider)}.`
        : "MindScript Studio couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from MindScript Studio.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · MindScript Studio</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("MindScript Studio couldn't finish connecting to "+PROVIDER+"."):"MindScript Studio couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from MindScript Studio."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("MindScript Studio is now connected to "+PROVIDER+"."):"MindScript Studio is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// mindscript_change: the upstream wordmark was 19 hand-drawn letterform paths spelling
// "OpenCode". Text keeps it legible and correct without redrawing glyphs.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 42" fill="none" aria-label="MindScript" role="img">
        <text x="0" y="31" font-family="Arial Black, Helvetica Neue, Arial, sans-serif" font-weight="900" font-size="30" letter-spacing="-0.5" fill="var(--oc-icon-base)">MINDSCRIPT</text>
      </svg>`
