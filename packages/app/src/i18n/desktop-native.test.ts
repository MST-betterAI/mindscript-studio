...
describe("desktop native translations", () => {
  test("offers English only", () => {
    expect(DESKTOP_NATIVE_LOCALES).toEqual(["en"])
    expect(DESKTOP_NATIVE_LOCALES.map((locale) => DESKTOP_NATIVE_LABELS[locale])).toEqual(["English"])
  })
...
describe("desktop native locale detection", () => {
  test("falls back to English for any preference list", () => {
